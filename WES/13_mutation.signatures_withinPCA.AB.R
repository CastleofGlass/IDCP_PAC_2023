...
set.seed(13)
row_ha = rowAnnotation(group=groups[,c('Mutation_group')],
                       col=list(Mutation_group = brewer.pal(7,"Set1")[groups$Mutation_group]) )
...
pdf(file.path(result_dir, "Mutation_signature_weight_withinPCA_AB.pdf"), width = 8, height = 6)
...
pdf(file.path(result_dir, "Mutation_signature_weight_withinPCA_AB_ClusterRow.pdf"), width = 8, height = 6)
Heatmap(mat, name = "weight",
        col = f1,
        cluster_rows = T,
        right_annotation = row_ha)
