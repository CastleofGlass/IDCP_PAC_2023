...
# 筛选PCA， adenocarcinoma
groups=groups[groups$group=="adenocarcinoma",]  
groups
...
result_dir="13_mutation_signatures_withinPCA_AB"
...
ggbarplot(df_long, x = "HXID",y = "value",
  ...
  ggtitle("Somatic Signatures for PCA")
ggsave(file.path(result_dir, "Somatic_Signatures_PCA.pdf"), width = 5 , height = 5)
...
ggbarplot(df_long, x = "HXID",y = "value",
  ...
  ggtitle("Somatic Signatures for PCA")
ggsave(file.path(result_dir, "Somatic_Signatures_for_PCA_Proportion.pdf"), width = 5 , height = 5)
...
pdf(file.path(result_dir, "Mutation_signature_MergeSignature_weight_withinPCA_AB.pdf"), width = 6, height = 3)
...
pdf(file.path(result_dir, "Mutation_signature_MergeSignature_weight_withinPCA_AB_ClusterCol.pdf"), width = 6, height = 3)
Heatmap(mat, name = "weight",
        col = f1,
        cluster_rows = F,
        cluster_columns = T,
...
write.csv(df,file.path(result_dir,"all.22.PCA.samples.cosmic.signatures.csv"),quote = F,row.names = F)
...
ggsave(file.path(result_dir, "Boxplot_ttest_AB_group_each_signature_withinPCA.pdf"), width = 8, height = 4)
...
ggsave(file.path(result_dir,"all.22.PCA.samples.cosmic.signatures.pvalue.between.A.and.B.pdf"),width = 6,height = 6)
...
pdf(file.path(result_dir, "Mutation_signature_weight_real_AB_ClusterRow_withinPCA.pdf"), width = 5, height = 2.5)
Heatmap(mat, name = "weight",
        col = f1,
        cluster_rows = F)
dev.off()
