...
pan39_gene=read.table("data/PAN39.gene.list.txt",stringsAsFactors = F,header = F)
pan39_gene=pan39_gene$V1
...
for(i in 1:nrow(samples)){
  ...
  gene_overlap=intersect(idcp_df$Gene,pan39_gene)
  print(length(gene_overlap))
  idcp_df=idcp_df[match(gene_overlap,idcp_df$Gene),]
  ...
  overlap_2=intersect(ade_df$Gene,pan39_gene)
  print(length(overlap_2))
  ade_df=ade_df[match(overlap_2,ade_df$Gene),]
...
write.csv(result,file.path(outdir,"sequenza_gene_level_cnv_overlapped.0518.csv"),quote = F,row.names = F)



df=read.csv(file.path(outdir,"sequenza_gene_level_cnv_overlapped.0518.csv"),stringsAsFactors = F,header = T)
...
write.csv(df,file.path(outdir,"sequenza_gene_level_cnv_overlapped.update.0518.csv"),quote = F,row.names = F)
...
ggsave(file.path(outdir,paste("sequenza","total","gene_level_cnv.0518.pdf",sep=".")),width = 6,height = 6)
...
ggsave(file.path(outdir,paste("sequenza","gain","gene_level_cnv.0518.pdf",sep=".")),width = 6,height = 6)
...
ggsave(file.path(outdir,paste("sequenza","loss","gene_level_cnv.0518.pdf",sep=".")),width = 6,height = 6)



ggscatter(df,x="GAIN_CNV_percentage",y="LOSS_CNV_percentage",label = "hxid")+xlim(0,100)+ylim(0,100)
ggsave(file.path(outdir,paste("sequenza","loss_vs_loss","gene_level_cnv.shared.0518.pdf",sep=".")),width = 6,height = 6)
...
ggsave(file.path(outdir,paste("SNV",".sequencza.gene_level_cnv.shared.0518.pdf",sep=".")),width = 6,height = 6)


write.csv(snv[,c("hxid","share_snv_percentage","TOtal_CNV_percentage")],file.path(outdir,"snv_cnv_shared.percentage.0518.csv"),quote = F,row.names = F)
