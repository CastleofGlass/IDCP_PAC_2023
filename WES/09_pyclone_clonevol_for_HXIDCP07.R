...
# 删除一些cluster
dat[1:2,]
dat <- dat %>% dplyr::filter(cluster_id != "4")
...
pdf(paste("09_pyclone_clonevol/",args,".variant.cluster.vaf.rm_clst.pdf",sep=""), width = 7, height = 7, useDingbats = FALSE, title='')
pp <- plot.variant.clusters(result,
                            cluster.col.name = 'cluster',
                            show.cluster.size = FALSE,
                            cluster.size.text.color = 'blue',
...
#### use ccf
pdf(paste("09_pyclone_clonevol/",args,".cluster.flow.ccf.rm_clst.pdf",sep=""))
...
plot.clonal.models(y_vaf,
                   ...
                   out.dir = file.path("09_pyclone_clonevol/",paste(args,"mono_vaf_rm_clst",sep="_")),
                   out.format = 'pdf',
...
plot.clonal.models(y_ccf,
                   ...
                   disable.cell.frac = FALSE,
                   # output figure parameters
                   out.dir = file.path("09_pyclone_clonevol",paste(args,"mono_ccf_rm_clst",sep="_")),
                   out.format = 'pdf',
                   overwrite.output = TRUE,width = 12,
                   height = 6,
                   # vector of width scales for each panel from left to right
...
plot.clonal.models(y_vaf,
                   ...
                   disable.cell.frac = FALSE,
                   # output figure parameters
                   out.dir = file.path("09_pyclone_clonevol/",paste(args,"poly_vaf_rm_clst",sep="_")),
                   out.format = 'pdf',
                   overwrite.output = TRUE,width = 12,
                   height = 6,
                   # vector of width scales for each panel from left to right
