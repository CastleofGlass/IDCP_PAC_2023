...
files=list.files(path=file.path("05_run_pyclone",args,"tables"),pattern = "loci.tsv",all.files = T,full.names = T)
...
## change name
pos2=match(dat_used$sample_id,samples$Tumor)
dat_used=cbind(dat_used,samples[pos2,])
...
pdf(paste("06_clonevol/",args,".variant.cluster.ccf.pdf",sep=""), width = 7, height = 7, useDingbats = FALSE, title='')
...
pdf(paste("06_clonevol/",args,".variant.cluster.vaf.pdf",sep=""), width = 7, height = 7, useDingbats = FALSE, title='')
pp <- plot.variant.clusters(result,
                            cluster.col.name = 'cluster',
                            show.cluster.size = FALSE,
                            cluster.size.text.color = 'blue',
...
#### use ccf
pdf(paste("06_clonevol/",args,".cluster.flow.ccf.pdf",sep=""))
plot.cluster.flow(result, vaf.col.names = ccf_col_name,
                  sample.names = ccf_col_name)
dev.off()

#### use vaf
pdf(paste("06_clonevol/",args,".cluster.flow.vaf.pdf",sep=""))
...
plot.pairwise(result, col.names = vaf_col_name,
              out.prefix = paste("06_clonevol/",args,".variants.pairwise.plot.vaf",sep="")
              )

plot.pairwise(result, col.names = ccf_col_name,
              out.prefix = paste("06_clonevol/",args,".variants.pairwise.plot.ccf",sep="")
)
...
plot.clonal.models(y_vaf,
                   ...
                   out.dir = file.path("06_clonevol/",paste(args,"mono_vaf",sep="_")),
                   out.format = 'pdf',
...
plot.clonal.models(y_ccf,
                   ...
                   disable.cell.frac = FALSE,
                   # output figure parameters
                   out.dir = file.path("06_clonevol",paste(args,"mono_ccf",sep="_")),
                   out.format = 'pdf',
                   overwrite.output = TRUE,width = 12,
                   height = 6,
                   # vector of width scales for each panel from left to right
...
plot.clonal.models(y_vaf,
                   ...
                   disable.cell.frac = FALSE,
                   # output figure parameters
                   out.dir = file.path("06_clonevol/",paste(args,"poly_vaf",sep="_")),
                   out.format = 'pdf',
                   overwrite.output = TRUE,width = 12,
                   height = 6,
                   # vector of width scales for each panel from left to right
...
plot.clonal.models(y_ccf,
                   ...
                   disable.cell.frac = FALSE,
                   # output figure parameters
                   out.dir = file.path("06_clonevol/",paste(args,"poly_ccf",sep="_")),
                   out.format = 'pdf',
                   overwrite.output = TRUE,width = 12,
                   height = 6,
                   # vector of width scales for each panel from left to right
