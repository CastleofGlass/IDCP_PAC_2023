...
snv$hxid_new=gsub("HXIDCP-","",snv$hxid)

ggscatter(snv,x="share_snv_percentage",y="TOtal_CNV_percentage",label = "hxid_new")+xlim(0,100)+ylim(0,100)+xlab("Shared SNV percentage(%)")+ylab("Shared CNV percentage(%)")
ggsave(file.path(outdir,paste("SNV",".sequencza.gene_level_cnv.shared_20210609.pdf",sep="")),width = 6,height = 6)
...
pdf(file.path(outdir,"snv.cnv.shared.hca.20210609.pdf",sep=""),width = 6,height = 6)
